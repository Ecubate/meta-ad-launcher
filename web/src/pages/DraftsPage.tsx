...
export function DraftsPage() {
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-muted text-sm mb-5">Saved launches you can return to and launch later.</p>
          ...
          {drafts.map((d) => (
            <div key={d.id} className="flex items-center justify-between border border-line rounded-lg px-4 py-3 bg-surface/40">
              <div>
                ...
                <div className="text-xs text-muted">{d.payload?.rows?.length ?? 0} ads</div>
              </div>
